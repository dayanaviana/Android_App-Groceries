    ...
    SQLiteOpenHelper(mContext, Config.DATABASE_NAME, null, Config.DATABASE_VERSION)
{
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        var sql = "CREATE TABLE IF NOT EXISTS $TABLE_NAME(" +
                ...
                ")"
        db?.execSQL(sql)
        Log.d("myApp", "DBHelper: Table cart created")
    }
    ...
    fun addCartItem(p: Product): Long {
        var insertedID: Long = -1
            ...
            Log.d("myApp", "DBHelper: CART_TABLE item inserted id=$insertedID.")
        }else{
            Log.d("myApp", "DBHelper: CART_TABLE already has the item.")
        }
        return insertedID
    }
